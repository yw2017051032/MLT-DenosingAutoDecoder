...
df1=pd.read_csv(filepath_or_buffer='../data/training_set.csv',encoding='utf-8')
...
encoded = Dense(nb_hidden, activation='tanh')(imputation_input)
# imputation_output = Dense(output_dim1, activation='sigmoid',name='imputation_output')(encoded)


# classification_input=Input(shape=(input_dim2,),name='classification_input')
# hidden = Dense(15, activation='relu')(classification_input)
# x = keras.layers.concatenate([encoded, hidden])
# # x=hidden
# classification_output = Dense(output_dim2, activation='sigmoid',name='classification_output')(x)


classification_input=Input(shape=(input_dim2,),name='classification_input')
hidden = Dense(15, activation='tanh')(classification_input)
classification_output = Dense(output_dim2, activation='softmax',name='classification_output')(hidden)
x = keras.layers.concatenate([encoded, classification_output ])
imputation_output = Dense(output_dim1, activation='sigmoid',name='imputation_output')(x)



autoencoder = Model(input=[imputation_input,classification_input], output=[imputation_output,classification_output])
autoencoder.compile(optimizer='adam',
                    loss={'imputation_output': 'binary_crossentropy', 'classification_output': 'binary_crossentropy'},
                    loss_weights={'imputation_output': 1., 'classification_output': 0.2},
                    metrics={'imputation_output':'mse','classification_output':'acc'})
# autoencoder.summary()
#adadelta
...
autoencoder.fit(x={'imputation_input':x_train_imputation,'classification_input':x_train},
                y={'imputation_output':x_train,'classification_output':y_train},
                nb_epoch=epochs,batch_size=batch_size, shuffle=True,
                verbose=1,validation_data=({'imputation_input':x_validate_imputation,
                                      'classification_input':x_validate},{'imputation_output':x_validate,
                                      'classification_output':y_validate}))


imputation,classification=autoencoder.predict(x={'imputation_input':x_train_imputation,'classification_input':x_train})
...
df=pd.read_csv(filepath_or_buffer='../data/training_set.csv',encoding='utf-8')
# df['GENDER']=df['GENDER'].map(mapping)
df=df.drop(['MEMBER_NO','class'],axis=1)
df=df.fillna(df.mean())
print('均值填补_mse:',str(metrics.mean_squared_error(x_test,np.array(df))))
for i,j in zip(loc1[0],loc1[1]):
    x_train[i][j]=imputation[i][j]
print('模型imputation_mse:',str(metrics.mean_squared_error(x_test,x_train)))
