...
missing_rate=0.1
df1=pd.read_csv(filepath_or_buffer='../data/training_setmissing_rate_'+str(missing_rate)+'missing_features20.csv',encoding='utf-8')
df=df1
...
decoded = Dense(40+14, activation='tanh')(encoded)
decoded = Dense(40+7, activation='tanh')(decoded)
...
# x = keras.layers.concatenate([decoded , hidden])
# imputation_output = Dense(40, activation='tanh',name='imputation_output')(x)



autoencoder = Model(input=[imputation_input], output=[imputation_output])
autoencoder.compile(loss='binary_crossentropy', optimizer='adadelta',metrics={'imputation_output':'mse'})
# autoencoder.summary()
...
autoencoder.fit(x={'imputation_input':x_train},y={'imputation_output':x_test},
                nb_epoch=epochs,batch_size=batch_size,shuffle=True,verbose=1,
                validation_data=({'imputation_input':x_validate},{'imputation_output':x_validate}))


imputation=autoencoder.predict(x={'imputation_input':x_train})

autoencoder.save(filepath='../model/DAE'+str(missing_rate)+'.h5')
...
# x_train.to_csv(path_or_buf='../imputed-data/imputed-data-DAE-'+str(missing_rate)+'.csv',index=False,encoding='utf-8')
#
# df.to_csv(path_or_buf='../imputed-data/imputed-data-mean-'+str(missing_rate)+'.csv',index=False,encoding='utf-8')
