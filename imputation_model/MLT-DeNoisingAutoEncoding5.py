...
# autoencoder.compile(loss={'imputation_output':'binary_crossentropy','classification_output':'binary_crossentropy'}, optimizer='adadelta',loss_weights={'imputation_output':2.5,'classification_output':0.5})
autoencoder.compile(loss={'imputation_output':my_loss,'classification_output':'categorical_crossentropy'}, optimizer='adadelta',loss_weights={'imputation_output':0.5,'classification_output':0.5})
# autoencoder.summary()
...
tensorboard=keras.callbacks.TensorBoard(log_dir='./tensorboard/log2')
...
autoencoder.fit(x={'imputation_input':x_train},y={'imputation_output':x_test,'classification_output':y_test},
                nb_epoch=epochs,batch_size=batch_size,shuffle=True,verbose=1,callbacks=[early_stopping,tensorboard],
...
autoencoder.save(filepath='../model/MLTDAE'+str(missing_rate)+'.h5')
...
rmse=np.sqrt(metrics.mean_squared_error(x_test,x_train))
print('模型imputation_rmse:%.4f'%(rmse))



x_train=pd.DataFrame(x_train)

x_train.to_csv(path_or_buf='../imputed-data/imputed-data-MLTDAE-'+str(missing_rate)+'.csv',index=False,encoding='utf-8')
